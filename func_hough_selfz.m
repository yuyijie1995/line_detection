function [ x3,x4,y3,y4,k,b ] = func_hough_selfz( select_point_zz )
%UNTITLED4 Summary of this function goes here
%   Detailed explanation goes here
%  rho = x*cos(theta) + y*sin(theta).
theta_z=-pi/18*2:-pi/270:-pi/18*8;
%imag=imread('C:\Users\Single-Dog\Desktop\车道线识别程序M版本\图片集\lane_33087gz.bmp');
...
%theta_z_sin=sin(theta_z);
 zx=select_point_zz(:,1);
 zy=select_point_zz(:,2);
...
for i=1:1:width
    for k=1:a
    p= round(zx(i)*cos(pi*k/180)+zy(i)*sin(pi*k/180));
    if(p>0)
        s(k,d+p)=s(k,d+p)+1;%（a，p）相应的累加器单元加一        
        z{k,d+p}=[z{k,d+p},[zx(i),zy(i)]'];%存储点坐标
    else
        ap=abs(p)+1;%若p小于0，则将点存储在（0，d）空间     
        s(k,ap)=s(k,ap)+1;%(a，p)相应的累加器单元加一          
        z{k,ap}=[z{k,ap},[zx(i),zy(i)]'];%存储
    end
    end
end
...
for i=1:width
    for jn=1:high
     ...
     %   hold on;
        ...
        y4=k*x4+b;
        x3=(y3-b)/k;
    %    plot([x3,x4],[y3,y4],'r','linewidth',4.5)
